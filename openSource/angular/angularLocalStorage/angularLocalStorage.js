 ...
 * https://github.com/agrublev/angularLocalStorage
 ...
 gst: 1/14/2014  fixed unbind code to allow for proper unwatching and fixed
 bug where it was not saving data to store when the data became a null.
...
( function ( window, angular, undefined ) {
	...
	angular.module ( 'angularLocalStorage', ['ngCookies'] ) .factory ( 'storage', ['$parse', '$cookieStore', '$window', '$log', function ( $parse, $cookieStore, $window, $log ) {
		...
		var publicMethods = {
			// gst:
			boolModifiedByGeHc : true, 	// flag to check in caller to prevent new downloads breaking the app
			 ...
			 * @returns {*} - returns whatever the stored value is along with deregistration functon to pass to unbind [gst]
			 */
			bind: function ( $scope, key, opts ) {
				...
				var deregFunction = $scope.$watch ( key, function ( val ) {
					...
					if ( angular.isDefined ( val ) ) {
						...
						// no val
						// gst added here
						publicMethods.set ( storeName, null );
					}
				}, true );


				var objRet = { deregFunction : deregFunction,
								value : publicMethods.get ( storeName ) 
							  }
			 ...
			 * gst: @param deregFunction function ( from bind (  )  ) 
			 */
			unbind: function ( $scope,key,storeName, deregFunction  ) {
				storeName = storeName || key;

				$parse ( key ) .assign ( $scope, null );

				// orig doesnt unwatch: [gst] $scope.$watch ( key, function (  ) { } );
				// unwatch gary way
				deregFunction (  );
