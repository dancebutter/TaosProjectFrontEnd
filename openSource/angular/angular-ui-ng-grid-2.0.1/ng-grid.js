...
* aaa gst modified. fixed elm.focus() bug  to: if ( elm.focus != undefined ) elm.focus();
 *
 ***********************************************/

var boolGlobalAngularNgGridModifiedbyGe = true;	// flag for app to make sure that this version is loaded and not a newer non-ge version from the web
...
angular.module (
	'ngGrid',
	['ngGrid.services', 'ngGrid.directives', 'ngGrid.filters'] );
	...
	function (domUtilityService) {
		...
		return function($scope, elm) {
			...
			elm.bind('mousedown', function(){
				if ( elm.focus != undefined ) elm.focus();
				return true;
			});
...
angular.module("ngGrid").run(["$templateCache", function($templateCache) {
  ...
  $templateCache.put("gridTemplate.html",
    ...
    "        <div ng-style=\"rowStyle(row)\" ng-repeat=\"row in renderedRows\" ng-click=\"row.toggleSelected($event)\" class=\"ngRow\" ng-class=\"row.alternatingRowClass()\" ng-row>" +
	"        </div>" +
    "    </div>" +
    "</div>" +
